...
export const ContributionGraph = ({
  ...
  const weeks = useMemo(
    () => groupByWeeks(normalizedData, weekStart),
    [normalizedData, weekStart],
  );
  ...
  const year =
    normalizedData.length > 0
      ? getYear(parseISO(normalizedData[0].date))
      : new Date().getFullYear();

  const totalCount =
    typeof totalCountProp === "number"
      ? totalCountProp
      : normalizedData.reduce((sum, activity) => sum + activity.count, 0);
